/**
 * n8n Code node — prefer prebuilt `html` from the Next.js app.
 *
 * The app now sends UI-matching HTML in the webhook payload (`html` field).
 * This node must NOT rebuild the old "Food · 60 min · then 14 min car" format.
 *
 * Option A (recommended): pass through app HTML
 * Option B: if `html` missing, build with the fallback below
 */

function unwrap(item) {
  const b = item.body && typeof item.body === "object" ? item.body : null;
  return {
    email: item.email || b?.email,
    city: item.city || b?.city || "Jaipur",
    summary: item.summary ?? b?.summary ?? null,
    itinerary: item.itinerary || b?.itinerary || {},
    sources: item.sources || b?.sources || [],
    sent_at: item.sent_at || b?.sent_at,
    html: item.html || b?.html || null,
  };
}

function formatClockAmPm(hhmm) {
...
function spendDurationLabel(minutes) {
...
<style>
  ...
  html += `<h1>${esc(data.city || "Jaipur")} itinerary</h1>`;
  const interests = (itin.trip && itin.trip.interests) || [];
  const pace = (itin.trip && itin.trip.pace) || "";
  const paceLabel = pace === "moderate" ? "balanced" : pace;
  const daysN = itin.trip && itin.trip.num_days ? itin.trip.num_days : days.length;
  html += `<p class="meta">${esc(daysN)}-day ${esc(paceLabel)} plan`;
  if (interests.length) html += ` · ${esc(interests.join(", "))}`;
  html += `</p>`;
  if (data.summary) html += `<p class="meta">${esc(data.summary)}</p>`;

  for (const day of days) {
    const theme = themeFromStops(day) || day.theme || "";
...
// Prefer HTML built by the Next.js app (clocks, hours, relax notes).
const html =
  typeof data.html === "string" && data.html.includes("<html")
    ? data.html
    : buildHtml(data);
